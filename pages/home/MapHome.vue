<template>
  <div id="ThreeBaseContainer" @click="onMouseClick">
    ...
    <div
      v-if="targetPannelControl.isShow"
      class="row_nw_center_center target_box"
      @click.stop.prevent="() => {}"
    >
      <MapTargetPanel
    ...
    <div
      v-if="satellitePannel.isShow"
      class="row_nw_center_center satellite_pannel_box"
      @click.stop.prevent="() => {}"
    >
      <MapSatellitePanel
...
import {
  getSateRtOrbitRequest,
  getTarget,
  getReveiverStationsRequest,
  getTleByCatalogNumber,
} from "~/api/orbit";
...
export default defineComponent({
  ...
  setup() {
    ...
    function loadFont(fontName: any, fontWeight: any) {
      ...
      fontLoader.load(
        "/jslib/Three/fonts/" + fontName + "_" + fontWeight + ".typeface.json",
        (response: any) => {
          font = response;
        }
      );
      ...
      for (const key of objKeys) {
        ...
        if (featureLineobj && expriredLineobj) {
          if (currentStep > minsPerInterval) {
            ...
            // 更新全新轨道
            const now = Date.now();
            const positions = getOrbitPositionDatas(
              now,
              satrecObj.satrec,
              satrecObj.minsPerInterval
            );
            featureLineobj.geometry.setAttribute(
              "position",
            ...
            const positions2 = getOrbitPositionDatas(oldTime, satrecObj.satrec, 10);
            expriredLineobj.geometry.setAttribute(
              "position",
            ...
            if (currentStep % 60000 == 0) {
              ...
              featureLineobj.geometry.setAttribute(
                "position",
                new THREE.Float32BufferAttribute(featureArray, 3)
              );
        ...
        if (pointObj) {
          ...
          expriredLineobj.geometry.setAttribute(
            "position",
            new THREE.Float32BufferAttribute(expiredArray, 3)
          );
    ...
    const onMouseClick = (event: any) => {
      ...
      if (raycaster) {
        ...
        for (let jj = 0; jj < 10; jj++) {
          let sateIntersects = raycaster.intersectObjects(
            allSatelliteGroup.children[jj].children[2].children,
            false
          );
